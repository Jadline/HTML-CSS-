...
function Modal(){
    ...
    return(
        <div className ={styles.modalContainer}>
            <span className ={styles.confirmorder}>
            ...
            <p className ={styles.welcomemessage}>We hope you enjoy your food</p>

            {cartitems.length > 0 && <div className={styles.orderdata}>
            {cartitems.map((item,index) => 
                    <div className={styles.order} key={index}>
                
                    <div className={styles.orderinfo}>
                        <div className={styles.imagedisplayinfo}>
                            <img src={item.image.desktop} alt='order-photo' className={styles.modalimage}/>
                            <div>
                                <p className={styles.name}>{item.name}</p>
                                <p><span className={styles.quantity}>{item.quantity}x</span><span className={styles.productprice}>@${item.price.toFixed(2)}</span></p>
                            </div>
                        </div>
                    </div>
            ...
            <p className={styles.totalorder}><span>Order Total</span><span>${total_price}</span></p>
            <button className={styles.startbutton} onClick={startnewOrder}>Start New Order</button> 
        </div>
    )
}
